...
public class XLSToH2Database {
  ...
  /**
   * Reads an XLSX file, extracts the data, and loads it into an in-memory H2
   * database table.
   * The table name is derived from the XLSX file name provided in the
   * IntegrationConfigPOJO class.
   * The first row of the XLSX file is treated as the header row, and its values
   * are used as column names in the database table.
   * Each subsequent row is inserted as a new row in the database table.
   * The XLSX file is assumed to have one sheet, specified by the sheetIndex
   * parameter (0-based).
   * The XLSX file path and other configuration details are fetched from the
   * IntegrationConfigPOJO class.
   *
   */
  ...
  /**
   * Creates a table in the database with the specified table name and column
   * names derived from the provided headers array.
   * Each header is treated as a column name, and all columns are defined as
   * VARCHAR(255) data type.
   *
   * @param connection The database connection.
   * @param tableName  The name of the table to be created.
   * @param headers    An array of column names derived from the XLSX file header
   *                   row.
